...
test('explains why an exact BBL is absent from the published inventory', async ({
  ...
  await page.route(
    /\/v1\/parcel-intel\/official-parcel\/\d+\/sales-comparables$/,
    async (route) => {
  ...
  await page
    ...
    .click();
  await expect(
    page.getByTestId('parcel-sales-comparables-ready'),
  ).toContainText('609 OVINGTON AVENUE');
  await expect(
    page.getByTestId('parcel-sales-comparables-ready'),
  ).toContainText('not an appraisal');
  await page
